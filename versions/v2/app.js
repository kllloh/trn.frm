...
function removeNode(id) {
  ...
  state.nodes.forEach(n => { if (n.inputFrom === id) n.inputFrom = null })
...
function topoSort() {
  ...
  const pass   = () => {
    ...
    for (const n of state.nodes) {
      ...
      if (n.inputFrom === null || done.has(n.inputFrom)) {
...
function updatePortStyles() {
  const hasInput  = new Set(state.nodes.filter(n => n.inputFrom !== null).map(n => n.id))
  const hasOutput = new Set(state.nodes.filter(n => n.inputFrom !== null).map(n => n.inputFrom))
  for (const n of state.nodes) {
...
function normalizeChainMarquees(node) {
  ...
  const inherited = root.marquee ?? group.find(n => n.marquee)?.marquee ?? null

  root.marquee = inherited ? { ...inherited } : null
  for (const n of group) {
    if (n.id !== root.id) n.marquee = null
    updateNodeHint(n)
  }
}
...
function marqueeHit(node, sx, sy) {
  if (!node?.marquee || node.inputFrom !== null) return null
...
canvasWrap.addEventListener('mousedown', e => {
  ...
  // Only the first/root node in a chain can create or own a marquee.
  const sel = state.nodes.find(n => n.id === state.selectedId)
  if (sel?.inputFrom !== null) return
...
main.addEventListener('click', e => {
  ...
  if (!e.target.closest('.effect-node') && !e.target.closest('#canvas-wrap')) {
...
function render() {
  ...
  requestAnimationFrame(() => {
    ...
    for (const seed of sorted) {
      ...
      const component = connectedNodes(seed)
        .sort((a, b) => sorted.indexOf(a) - sorted.indexOf(b))
      component.forEach(n => handled.add(n.id))

      const nodeOutputs = new Map()
      const renderedIds = new Set()

      for (const node of component) {
        // Connected nodes share one marquee. This lets an upstream node render
        // into a marquee owned by a downstream node.
        const mq = effectiveMarquee(node)
        if (!mq || mq.w < 1 || mq.h < 1) continue

        // Resolve source: upstream output inside this component, or the current
        // visible canvas state after earlier independent components.
        let src
      ...
      // Composite only final visible outputs for this component. Intermediate
      // nodes feed downstream nodes; terminal nodes hit the image.
      for (const node of component) {
...
overlayToggle.addEventListener('click', () => {
  ...
  document.querySelectorAll('.effect-node').forEach(el => {
